...
public:
	...
	// 初始化（可调用一次，不调用则使用默认）
	static void init(const std::string &filename = "/home/lihaoqian/project/clouddisk_v2/forward_part/logs/app.log",
...
#define LOG_ERROR_RID(rid, fmt, ...) SPDLOG_LOGGER_ERROR(GlobalLogger::get(), "[rid:{}] " fmt, rid, ##__VA_ARGS__)

package main

import (
	"context"
	"errors"
	"fmt"
	"net/http"
	"os"
	"os/signal"
	"strconv"
	"strings"
	"syscall"
	"time"

	accountpb "go_test/backword_part/account_server/account_srv/protobuf"
	filepb "go_test/backword_part/file_server/file_srv/protobuf"
	"go_test/backword_part/mcp_server/apikey"
	"go_test/backword_part/mcp_server/handler"
	"go_test/backword_part/mcp_server/middleware"
	"go_test/internal"

	"github.com/mark3labs/mcp-go/server"
	"go.uber.org/zap"
	"google.golang.org/grpc"
	"google.golang.org/grpc/credentials/insecure"
)

// ================= 认证 claims ==================== ==========
type ctxKey string

const claimsKey ctxKey = "claims"

type Claims struct {
	UserID   int32
	Username string
}

func requireClaims(ctx context.Context) (*Claims, error) {
	v := ctx.Value(claimsKey)
	c, ok := v.(*Claims)
	if !ok || c == nil {
		return nil, fmt.Errorf("missing auth claims in context")
	}
	if c.UserID <= 0 || strings.TrimSpace(c.Username) == "" {
		return nil, fmt.Errorf("invalid auth claims")
	}
	return c, nil
}

// ========== 配置 ==========
const (
	defaultTransport      = "sse"
	defaultBaseURL        = ""     // 单实例直连：建议为空（返回 path，避免 URL 拼错）
	defaultBasePath       = "/mcp" // 你的静态挂载点
	defaultDialTimeout    = 5 * time.Second
	defaultRequestTimeout = 15 * time.Second

	// SSE 保活：避免中间链路 idle 超时断开
	defaultKeepAlive         = true
	defaultKeepAliveInterval = 15 * time.Second

	// Rate Limiting
	defaultRateLimit = 60 // 每分钟 60 次请求
)

type config struct {
	transport      string
	httpAddr       string
	baseURL        string
	basePath       string
	accountAddr    string
	fileAddr       string
	consulAddr     string
	dialTimeout    time.Duration
	requestTimeout time.Duration
	rateLimit      int
}

type serviceClients struct {
	accountConn *grpc.ClientConn
	fileConn    *grpc.ClientConn
	account     accountpb.AccountServiceClient
	file        filepb.FileServiceClient
}

func (c *serviceClients) Close() {
	if c.accountConn != nil {
		_ = c.accountConn.Close()
	}
	if c.fileConn != nil {
		_ = c.fileConn.Close()
	}
}

// ========== main ==========
func main() {
	rootCtx := context.Background()

	cfg, err := loadConfig()
	if err != nil {
		internal.Logger.Error("failed to load config", zap.Error(err))
		return
	}

	clients, err := newServiceClients(cfg)
	if err != nil {
		internal.Logger.Error("mcp grpc clients init failed", zap.Error(err))
		return
	}
	defer clients.Close()

	// 初始化 API Key 管理器
	apiKeyManager := apikey.NewManager()
	rateLimiter := apikey.NewRateLimiter(cfg.rateLimit)

	// 创建认证中间件
	authMiddleware := middleware.NewAPIKeyAuth(apiKeyManager, rateLimiter, internal.Logger)

	// 创建 API Key Handler
	apiKeyHandler := handler.NewAPIKeyHandler(apiKeyManager, internal.Logger)

	mcpServer := server.NewMCPServer(
		"clouddisk-backword",
		"1.0.0",
		server.WithToolCapabilities(true),
		server.WithRecovery(),
	)

	registerTools(mcpServer, clients, cfg.requestTimeout)

	if strings.EqualFold(cfg.transport, "stdio") {
		if err := server.ServeStdio(mcpServer); err != nil {
			internal.Logger.Error("mcp stdio server error", zap.Error(err))
		}
		return
	}

	// SSE options：关键是不要乱拼 full URL，单实例建议返回 path
	sseOpts := []server.SSEOption{
		server.WithStaticBasePath(cfg.basePath),
		server.WithUseFullURLForMessageEndpoint(false),
		server.WithKeepAlive(defaultKeepAlive),
		server.WithKeepAliveInterval(defaultKeepAliveInterval),

		// 每个 HTTP 请求（/sse、/message）按 header 注入 claims
		server.WithSSEContextFunc(func(ctx context.Context, r *http.Request) context.Context {
			// 从 API Key 认证中间件获取用户信息
			userID := middleware.GetUserIDFromContext(ctx)
			if userID > 0 {
				// 从数据库查询用户名（简化示例，实际应该缓存）
				cl := &Claims{
					UserID:   int32(userID),
					Username: fmt.Sprintf("user_%d", userID),
				}
				return context.WithValue(ctx, claimsKey, cl)
			}

			// 兼容旧的 Header 方式（用于测试）
			uidStr := strings.TrimSpace(r.Header.Get("X-User-Id"))
			uname := strings.TrimSpace(r.Header.Get("X-Username"))
			internal.Logger.Info("mcp incoming headers",
				zap.String("path", r.URL.Path),
				zap.String("x-user-id", uidStr),
				zap.String("x-username", uname),
			)
			if uidStr == "" || uname == "" {
				return ctx
			}
			uid64, err := strconv.ParseInt(uidStr, 10, 32)
			if err != nil || uid64 <= 0 {
				return ctx
			}
			cl := &Claims{UserID: int32(uid64), Username: uname}
			return context.WithValue(ctx, claimsKey, cl)
		}),
	}

	if strings.TrimSpace(cfg.baseURL) != "" {
		sseOpts = append(sseOpts,
			server.WithBaseURL(cfg.baseURL),
			server.WithUseFullURLForMessageEndpoint(true),
		)
	}

	sseServer := server.NewSSEServer(mcpServer, sseOpts...)

	// 自定义 mux：同端口挂 /health + MCP + API Key 管理
	mux := http.NewServeMux()

	// Health check（无需认证）
	mux.HandleFunc("/health", func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write([]byte("ok"))
	})

	// API Key 管理接口（无需认证，但应该在生产环境中添加管理员认证）
	mux.HandleFunc("/api/apikey/generate", apiKeyHandler.HandleGenerateAPIKey)
	mux.HandleFunc("/api/apikey/get", apiKeyHandler.HandleGetAPIKey)
	mux.HandleFunc("/api/apikey/revoke", apiKeyHandler.HandleRevokeAPIKey)

	// 当前用户信息（需要 API Key 认证）
	mux.Handle("/api/apikey/me", authMiddleware.Middleware(http.HandlerFunc(apiKeyHandler.HandleGetCurrentUser)))

	// MCP 服务（需要 API Key 认证）
	mux.Handle("/", authMiddleware.Middleware(sseServer))

	httpSrv := &http.Server{
		Addr:              cfg.httpAddr,
		Handler:           mux,
		ReadHeaderTimeout: 5 * time.Second,
	}

	// 定期清理旧的 Rate Limit 记录
	go func() {
		ticker := time.NewTicker(1 * time.Hour)
		defer ticker.Stop()

		for {
			select {
			case <-rootCtx.Done():
				return
			case <-ticker.C:
				deleted, err := rateLimiter.CleanupOldRecords(rootCtx, 24)
				if err != nil {
					internal.Logger.Error("Failed to cleanup rate limit records", zap.Error(err))
				} else if deleted > 0 {
					internal.Logger.Info("Cleaned up rate limit records", zap.Int64("count", deleted))
				}
			}
		}
	}()

	// 优雅退出
	shutdown := make(chan os.Signal, 1)
	signal.Notify(shutdown, syscall.SIGINT, syscall.SIGTERM)
	go func() {
		<-shutdown
		ctx, cancel := context.WithTimeout(rootCtx, 5*time.Second)
		defer cancel()

		_ = sseServer.Shutdown(ctx)
		_ = httpSrv.Shutdown(ctx)
	}()

	internal.Logger.Info("mcp server listening",
		zap.String("address", cfg.httpAddr),
		zap.String("base_path", cfg.basePath),
		zap.String("base_url", cfg.baseURL),
		zap.Int("rate_limit", cfg.rateLimit),
	)

	if err := httpSrv.ListenAndServe(); err != nil && !errors.Is(err, http.ErrServerClosed) {
		internal.Logger.Error("mcp http server error", zap.Error(err))
	}
}

// ========== 服务发现 / 配置 ==========
func FindServer(key string) (string, error) {
	srvHost, srvPort, err := internal.DiscoverService(key)
	if err != nil {
		internal.Logger.Error("find server failed", zap.String("key", key), zap.Error(err))
		return "", fmt.Errorf("discover service failed: %w", err)
	}
	return fmt.Sprintf("%s:%d", srvHost, srvPort), nil
}

func loadConfig() (config, error) {
	ip := internal.ViperConf.ConsulConfig.MCPSrv.Host
	port := internal.ViperConf.ConsulConfig.MCPSrv.Port
	addr := fmt.Sprintf("%v:%d", ip, port)

	accountADDR, err := FindServer("account_srv")
	if err != nil {
		return config{}, err
	}
	fileADDR, err := FindServer("file_srv")
	if err != nil {
		return config{}, err
	}
	consulADDR := internal.ViperConf.ConsulConfig.Host + ":" + internal.ViperConf.ConsulConfig.Port

	cfg := config{
		transport:      defaultTransport,
		httpAddr:       addr,
		baseURL:        defaultBaseURL,
		basePath:       defaultBasePath,
		accountAddr:    accountADDR,
		fileAddr:       fileADDR,
		consulAddr:     consulADDR,
		dialTimeout:    defaultDialTimeout,
		requestTimeout: defaultRequestTimeout,
		rateLimit:      defaultRateLimit,
	}
	cfg.basePath = normalizeBasePath(cfg.basePath)
	return cfg, nil
}

func normalizeBasePath(basePath string) string {
	basePath = strings.TrimSpace(basePath)
	if basePath == "" {
		return defaultBasePath
	}
	if !strings.HasPrefix(basePath, "/") {
		basePath = "/" + basePath
	}
	return strings.TrimRight(basePath, "/")
}

func newServiceClients(cfg config) (*serviceClients, error) {
	accountConn, err := dialGRPC(cfg.accountAddr, cfg.dialTimeout)
	if err != nil {
		return nil, fmt.Errorf("dial account grpc: %w", err)
	}
	fileConn, err := dialGRPC(cfg.fileAddr, cfg.dialTimeout)
	if err != nil {
		_ = accountConn.Close()
		return nil, fmt.Errorf("dial file grpc: %w", err)
	}

	return &serviceClients{
		accountConn: accountConn,
		fileConn:    fileConn,
		account:     accountpb.NewAccountServiceClient(accountConn),
		file:        filepb.NewFileServiceClient(fileConn),
	}, nil
}

func dialGRPC(addr string, timeout time.Duration) (*grpc.ClientConn, error) {
	ctx, cancel := context.WithTimeout(context.Background(), timeout)
	defer cancel()

	return grpc.DialContext(
		ctx,
		addr,
		grpc.WithTransportCredentials(insecure.NewCredentials()),
		grpc.WithBlock(),
	)
}
